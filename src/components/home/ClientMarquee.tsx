...
function MarqueeRow({ items, direction = 'left', speed = 40 }: { items: string[]; direction?: 'left' | 'right'; speed?: number }) {
  ...
  // Use CSS animation for buttery smooth marquee (no framer-motion overhead)
  const animationStyle = contentWidth > 0 ? {
    ...
    // Use translateX for hardware-accelerated animation
  ...
  return (
    ...
    >
      <div
        ref={rowRef}
        className="flex items-center gap-12 md:gap-16 lg:gap-20 w-max will-change-transform"
